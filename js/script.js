...
const error = {
  ...
  regPhone: 'Ввведите корректный номер',
};
const validationSubmit =
    //проверяем поля на форме
    $("#order_form").validate({
      onsubmit: true,
      errorClass: "validation_error error",
      validClass: "success",
      onkeyup: true,
      rules: {
        name: {
      ...
      submitHandler: function (form) {
        ...
        $.ajax({
          url: 'success.php',
          type: 'POST',
          data: {form: form, arr: arr},
          success: function (data) {
            jsonDecodeResult = JSON.parse(data);
            name = jsonDecodeResult.name;
            phone = jsonDecodeResult.phone;
            arr = jsonDecodeResult.arr;
            console.log(arr = jsonDecodeResult.arr)
...
$(".glow-button").click(function(){
  $(".comment-item").css('display','block')
  console.log(123)
})


$('#scroll_id').on('click',function(){
  scrollDown();
});
